...
"font: 75 10pt \"MS Shell Dlg 2\";")
    ...
    # A function that receives Dialog, client, main_win. The function gets the fields from the lines, does a validation, and send the details of the user to the server to check if the details are correct. The function would raise error labels when neccesarry
    def try_login(self, Dialog, client, main_win):
        username = self.lineEdit_username.text()
        password = self.lineEdit_password.text()

        if main_win.theAppConfig.isUsingDebugValues():
            username = main_win.theAppConfig.getUserName()
            password = main_win.theAppConfig.getPassword()
