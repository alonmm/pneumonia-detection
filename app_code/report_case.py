...
class Ui_Report_a_visit(object):
    ...
    def setupUi(self, Dialog, client, main_win, doctor_id):
        self._dlgPatient = Dialog
        self._wndMain = main_win
        self._iDoctor_id= doctor_id
        ...
        self.save_btn.clicked.connect(lambda: self.doWork(client))

        # self.progressBar = QtWidgets.QProgressBar(Dialog)
        # self.progressBar.setGeometry(QtCore.QRect(20, 485, 301, 475))
    ...
    def printPack(self,inParamsTuple):
        print (inParamsTuple)
    def doWork(self,client):
        inParamsTuple = (self.lineEdit_photo_path.text(),True)

        if self._wndMain.theAppConfig.isUsingDebugValues():
            inParamsTuple = (self._wndMain.theAppConfig.getStringValue('debug','xray_path'), True)

        if not inParamsTuple[0]:
            self.savePatient(None,client)

        else:
            self.printPack(inParamsTuple)
            self.workDlg = QtWidgets.QDialog()
            self.ui = progress_bar.WorkerDlg(self.workDlg)
            self.ui.setupUi(self.workDlg,client,sendCurl.send_curl,inParamsTuple,self.savePatient)
            self.workDlg.show()
    ...
    def report(self, Dialog, client, main_win, doctor_id):
        ...
        if not self.view_mode:
            if self.check_ID(client, ID, doctor_id):
                self.invalid_id_text.show()
                self.invalid_id_text.raise_()
                return
        ...
        try:
            ...
            if client.add_patient(full_name, ID, email, gender, birth_date, case_description, visit_date,  chance, doctor_id):
    ...
    def send_curl(self, photo_path):
        return sendCurl.send_curl(photo_path)
    ...
    def open_results_win(self, is_pneu):
        self.dialog = QtWidgets.QDialog()
        self.ui = results_win.Ui_results_win()
        self.ui.setupUi(self.dialog, is_pneu, True)
        self.dialog.show()
    ...
    def getPneumaniaProbability(self,tupleRes):
        res = str()
        if not tupleRes:
            return res
        if len(tupleRes) == 2 and tupleRes[0] == True:
            res = "Pneumania in " + str(tupleRes[1])
        elif len(tupleRes) == 2 and tupleRes[0] == False:
            res = "Normal in " + str(tupleRes[1])
        return res

    def savePatient(self,res_tuple_pneumania,client):
        full_name = self.lineEdit_full_name.text()
        ID = self.lineEdit_id.text()
        email = self.lineEdit_email.text()
        birthDate = self.birth_dateEdit.date()
        birthDate = str(str(birthDate.toPyDate()))

        visitDate = self.visit_dateEdit.date()
        visitDate = str(str(visitDate.toPyDate()))

        case_description = self.description_textEdit.toPlainText()
        photo_path = self.lineEdit_photo_path.text()

        chance = self.getPneumaniaProbability(res_tuple_pneumania)
        ...
        client.add_patient(full_name, ID, email, self.gender_text.text(),visitDate, case_description, birthDate, str(chance), self._iDoctor_id)

        self._wndMain.clear_table()
        self._wndMain.fill_table(client)
